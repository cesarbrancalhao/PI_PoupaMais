...
import { verificationModal } from "@/app/terminology/language/modals/verification";
...
export default function VerificationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!code || code.length !== 6) {
      setError(t(verificationModal.codeRequired));
      return;
    }
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError(t(verificationModal.invalidCode));
      }
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <input
              type="text"
              placeholder={t(verificationModal.codePlaceholder)}
              value={code}
          ...
          >
            {loading ? t(verificationModal.verifying) : t(verificationModal.verify)}
          </button>
        </form>
        ...
        <div className="mt-6 text-center">
          ...
          >
            {resending ? t(verificationModal.verifying) : t(verificationModal.resendCode)}
          </button>
        </div>

        <p className="text-xs text-gray-500 text-center mt-4">
          {t(verificationModal.expiresIn)}
        </p>
        <p className="text-xs text-gray-500 text-center mt-2">
          {t(verificationModal.checkSpam)}
        </p>
      </div>
    </div>
  );
